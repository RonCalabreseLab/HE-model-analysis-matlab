...
plot_one_phase = ...
    @(input_num, a_db, col_names, adjust_val) ...
    plotXRows(transpose(a_db(:, col_names) + adjust_val), ...
              input_num, input_names{input_num}, '', ...
              struct('command', 'plot', 'Vertical', 1, ...
                     'plotProps', ...
                     struct('LineStyle', '-', ...
                            'LineWidth', 2, ...
...
all_plots = {};
for input_num = 1:6
  all_plots{input_num} = plot_one_phase(input_num, hn_phase_peri_db, ...
                                        {'HN7', 'HN6', 'HN4', 'HN3'}, -1);
end
for input_num = 1:6
  all_plots = [ all_plots ...
                { plot_one_phase(input_num, hn_phase_sync_db, ...
                                 {'HN7', 'HN6', 'HN4', 'HN3'}, 0.5) } ];
end
hn_plot = ...
    plot_superpose(all_plots, {'Phase', 'HNs'}, '', ...
                   struct('noLegends', 1, 'noTitle', 1, ...
                          'fixedSize', [4 3]));

% now plot HE phases
he8_12_phase_sync_db = ...
    addColumns(renameColumns(he8_phase_sync_db(:, 'median'), 'median', 'HE8'), ...
               renameColumns(he12_phase_sync_db(:, 'median'), 'median', 'HE12'));

he8_12_phase_peri_db = ...
    addColumns(renameColumns(he8_phase_peri_db(:, 'median'), 'median', 'HE8'), ...
               renameColumns(he12_phase_peri_db(:, 'median'), 'median', 'HE12'));

all_plots = {};
for input_num = 1:6
  all_plots = [ all_plots ...
                { plot_one_phase(input_num, he8_12_phase_peri_db, ...
                                 {'HE12', 'HE8'}, 0) } ];
end
for input_num = 1:6
  all_plots = [ all_plots ...
                { plot_one_phase(input_num, he8_12_phase_sync_db, ...
                                 {'HE12', 'HE8'}, -0.5) } ];
end

he_plot = ...
    plot_superpose(all_plots, {'Phase', 'HEs'}, '', ...
                   struct('noLegends', 1, 'noTitle', 1, ...
                          'fixedSize', [4 3]));

plotFigure(plot_stack({hn_plot, he_plot}, [-0.4 0.7 NaN NaN], 'y', '', ...
                      struct('fixedSize', [4 5], 'xLabelsPos', 'bottom', ...
                             'xTicksPos', 'bottom', 'relativeSizes', [2 1])));
% => decided to use existing AI files for now.

% TODO:
% - Use row names in plotXRows?
