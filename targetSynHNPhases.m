...
plot_one_hns = ...
    @(input_num, a_db, adjust_val) ...
    plotXRows(transpose(a_db(:, {'HN7', 'HN6', 'HN4', 'HN3'}) + adjust_val), ...
...
all_plots = {};
for input_num = 1:6
  all_plots{input_num} = plot_one_hns(input_num, hn_phase_peri_db, -1);
end
for input_num = 1:6
  all_plots = [ all_plots ...
                { plot_one_hns(input_num, hn_phase_sync_db, 0.5) } ];
end
plotFigure(plot_superpose(all_plots, {'Phase', 'HNs'}, '', ...
                          struct('noLegends', 1, 'fixedSize', [4 3])))
