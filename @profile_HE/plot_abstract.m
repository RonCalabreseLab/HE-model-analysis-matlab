...
a_plot.plots{2} = annotate_plot(2, 'sync');
  ...
  if isfield(props, 'plotHNcolors')
    ...
    for hn_ind = 1:length(hn_nums)
      ...
      % Look for different synaptic multiplier forms in parameters
      hn_weight = ...
          hn_weight * ...
          getFieldDefault(...
              prof_props.params, ... % per HN multiplier 
              ['synS_mult_HE' num2str(he_num) '_HN' ...
               num2str(hn_nums(hn_ind))], ... % else
              getFieldDefault(...
                  prof_props.params, ... % per HE multiplier
                  ['synS_mult_HE' num2str(he_num)], ... % else
                  getFieldDefault(... % general multiplier for all HEs/HNs
                      prof_props.params, 'synS_mult', ...
                      1))); % otherwise multiply by 1
  ...
  annot_plots = {};
  if isfield(a_prof.intermediate_data, 'medianspikeraw')
    annot_plots = {...
  ...
  a_trace_plot = ...
      plot_superpose({...
        ...
        annot_plots{:}, hn_plots{:}}, {}, '', struct('noCombine', 1));
  end
end